-- Delete one invoice and only the financial history allocated to that invoice.
-- Shared supplier payments are retained and reduced to their remaining allocations.
...
begin
  ...
  insert into public.finance_audit_events(
    ...
    v_invoice.store_id,'store',v_invoice.store_id,'invoice_deleted',(select auth.uid()),
    jsonb_build_object(
