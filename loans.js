...
(function(){
  ...
  function annualRateFromMonthly(value){return round4(Number(value)*12);}
  function monthlyRateFromAnnual(value){return round4(Number(value)/12);}
  function rateLabel(value){return new Intl.NumberFormat('mn-MN',{maximumFractionDigits:4}).format(Number(value)||0);}
  ...
  function removeLoanOverlay(){revokeUrls();overlay()?.remove();document.body.classList.remove('loanFormOpen');}
  function closeLoanCreate(){removeLoanOverlay();state.files=[];state.draft=null;state.editingLoanId=null;}
  ...
  function editingLoan(){return state.loans.find(item=>String(item.id)===String(state.editingLoanId))||null;}
  function renderAnnualInterestPreview(){
    const target=document.getElementById('loanAnnualInterestPreview'),raw=fieldValue('loanInterest').trim(),monthly=Number(raw);if(!target)return;
    target.textContent=raw!==''&&Number.isFinite(monthly)&&monthly>=0?`Жилийн хүү: ${rateLabel(annualRateFromMonthly(monthly))}%`:'Жилийн хүү: —';
  }

  function loanRecordDraft(loan){
    return loan?{lenderType:loan.lender_type,lenderName:loan.lender_name,loanName:loan.loan_name,principal:Number(loan.principal),interest:Number(loan.annual_interest_rate),monthlyInterest:monthlyRateFromAnnual(loan.annual_interest_rate),startDate:loan.start_date,term:Number(loan.term_months),method:loan.repayment_method,paymentDay:Number(loan.payment_day)}:null;
  }

  function openLoanForm(prefill){
    const loan=editingLoan(),isEdit=Boolean(loan),data=prefill||loanRecordDraft(loan)||{lenderType:'bank',lenderName:'',loanName:'',principal:'',monthlyInterest:'',startDate:today(),term:''};
    removeLoanOverlay();
    const principalValue=data.principal===''?'':window.__nayadFormatMoneyInput?.(data.principal)||data.principal;
    const monthlyValue=data.monthlyInterest!==undefined?data.monthlyInterest:monthlyRateFromAnnual(data.interest);
    const contractSection=isEdit
      ?`<div class="loanEditNotice"><b>Гэрээний файл хэвээр хадгалагдана</b><span>${loan.documents.length?`${loan.documents.length} файл хавсаргасан`:'Гэрээ хавсаргаагүй'}</span></div>`
      :`<div class="loanFormLabel">Зээлийн гэрээ</div><div class="loanAttachmentChoices"><button type="button" onclick="document.getElementById('loanCameraInput').click()"><span>⌾</span><b>Зураг авах</b></button><button type="button" onclick="document.getElementById('loanGalleryInput').click()"><span>▧</span><b>Зургаас сонгох</b></button><button type="button" onclick="document.getElementById('loanPdfInput').click()"><span>PDF</span><b>PDF оруулах</b></button></div><input id="loanCameraInput" class="hide" type="file" accept="image/*" capture="environment" onchange="selectLoanFiles(this.files)"><input id="loanGalleryInput" class="hide" type="file" accept="image/*" multiple onchange="selectLoanFiles(this.files)"><input id="loanPdfInput" class="hide" type="file" accept="application/pdf" multiple onchange="selectLoanFiles(this.files)"><div id="loanFilePreview" class="loanFilePreview"></div><small class="loanFileHelp">Олон зураг нэг дор сонгож болно</small>`;
    const root=document.createElement('section');root.id='loanFormOverlay';root.className='loanFormOverlay';
    root.innerHTML=`<div class="loanFormHeader"><button type="button" onclick="closeLoanCreate()" aria-label="Буцах">←</button><span class="nayadWord"><span>N</span><span>A</span><span class="y">Y</span><span>A</span><span>D</span></span><i></i></div><div class="loanFormBody"><h1>${isEdit?'Зээл засах':'Шинэ зээл бүртгэх'}</h1><p>${isEdit?'Зээлийн мэдээллээ залруулна уу':'Зээлийн үндсэн мэдээллээ оруулна уу'}</p><div class="loanFormLabel">Зээл олгогчийн төрөл</div><input id="loanLenderType" type="hidden" value="${esc(data.lenderType||'bank')}"><div class="loanSegment"><button class="${data.lenderType==='bank'?'active':''}" data-value="bank" onclick="setLoanLenderType('bank')" type="button">Банк</button><button class="${data.lenderType==='nbfi'?'active':''}" data-value="nbfi" onclick="setLoanLenderType('nbfi')" type="button">ББСБ</button><button class="${data.lenderType==='person'?'active':''}" data-value="person" onclick="setLoanLenderType('person')" type="button">Хувь хүн</button></div><div class="field"><label>Байгууллага / хүний нэр</label><input id="loanLenderName" maxlength="120" value="${esc(data.lenderName)}" placeholder="Жишээ: ХААН банк"></div><div class="field"><label>Зээлийн нэр</label><input id="loanName" maxlength="120" value="${esc(data.loanName)}" placeholder="Жишээ: Бизнесийн зээл"></div><div class="loanFormGrid"><div class="field"><label>Авсан дүн</label><input id="loanPrincipal" data-money-input inputmode="decimal" value="${esc(principalValue)}" placeholder="10,000,000"></div><div class="field"><label>Сарын хүү</label><input id="loanInterest" type="number" inputmode="decimal" min="0" max="83.3333" step="0.0001" value="${esc(monthlyValue)}" placeholder="1.5" oninput="renderLoanAnnualInterest()"><small id="loanAnnualInterestPreview" class="loanAnnualInterestPreview">Жилийн хүү: —</small></div><div class="field"><label>Авсан огноо</label><input id="loanStartDate" type="date" value="${esc(data.startDate)}"></div><div class="field"><label>Хугацаа</label><input id="loanTerm" type="number" inputmode="numeric" min="1" max="600" value="${esc(data.term)}" placeholder="24 сар"></div></div>${contractSection}</div><div class="loanFormFooter"><button id="loanContinueBtn" class="primary full" type="button" onclick="continueLoanCreate()">Үргэлжлүүлэх</button></div>`;
    document.getElementById('app')?.appendChild(root);document.body.classList.add('loanFormOpen');window.__nayadPrepareMoneyInputs?.(root);renderAnnualInterestPreview();if(!isEdit)renderFilePreviews();
  }

  function showLoanCreate(){if(!canManage())return notify('Зээлийг зөвхөн эзэн эсвэл менежер бүртгэнэ.');closeLoanCreate();openLoanForm();}
  function showLoanEdit(loanId){
    if(!canManage())return notify('Зээлийг зөвхөн эзэн эсвэл менежер засна.');
    const loan=state.loans.find(item=>String(item.id)===String(loanId));if(!loan)return notify('Зээл олдсонгүй.');
    window.closeSheet?.();closeLoanCreate();state.editingLoanId=loan.id;openLoanForm();
  }
  ...
  function readLoanDraft(){
    const lenderType=fieldValue('loanLenderType'),lenderName=fieldValue('loanLenderName').trim(),loanName=fieldValue('loanName').trim(),principal=parseAmount(fieldValue('loanPrincipal')),monthlyInterestRaw=fieldValue('loanInterest').trim(),monthlyInterest=Number(monthlyInterestRaw),interest=annualRateFromMonthly(monthlyInterest),startDate=fieldValue('loanStartDate'),term=Number(fieldValue('loanTerm'));
    ...
    if(monthlyInterestRaw===''||!Number.isFinite(monthlyInterest)||monthlyInterest<0||interest>1000)return notify('Сарын хүүг зөв оруулна уу.');
    ...
    return {lenderType,lenderName,loanName,principal,monthlyInterest,interest,startDate,term,method:state.draft?.method||editingLoan()?.repayment_method||'annuity',paymentDay:state.draft?.paymentDay||Number(editingLoan()?.payment_day)||null};
  }
  ...
  function showScheduleStep(){
    ...
    const day=Math.min(Math.max(Number(draft.paymentDay)||Number(draft.startDate.split('-')[2])||1,1),31),method=draft.method||'annuity',isEdit=Boolean(editingLoan());
    root.innerHTML=`<div class="loanFormHeader"><button type="button" onclick="showLoanCreateFromDraft()" aria-label="Буцах">←</button><span class="nayadWord"><span>N</span><span>A</span><span class="y">Y</span><span>A</span><span>D</span></span><i></i></div><div class="loanFormBody"><h1>Төлөлтийн хуваарь</h1><p>${esc(draft.lenderName)} · ${amount(draft.principal)}</p><div class="loanFormLabel">Төлөх арга</div><input id="loanRepaymentMethod" type="hidden" value="${esc(method)}"><div class="loanMethodCards"><button class="${method==='annuity'?'active':''}" data-value="annuity" type="button" onclick="setLoanMethod('annuity')"><b>Тэнцүү төлөлт</b><small>Сар бүр ойролцоо ижил дүн</small></button><button class="${method==='equal_principal'?'active':''}" data-value="equal_principal" type="button" onclick="setLoanMethod('equal_principal')"><b>Үндсэн төлбөр тэнцүү</b><small>Эхний сар их, дараа нь буурна</small></button></div><div class="field"><label>Сар бүр төлөх өдөр</label><input id="loanPaymentDay" type="number" inputmode="numeric" min="1" max="31" value="${day}" oninput="renderLoanSchedulePreview()"></div><div id="loanSchedulePreview"></div></div><div class="loanFormFooter"><button id="loanSaveBtn" class="primary full" type="button" onclick="saveLoan()">${isEdit?'ӨӨРЧЛӨЛТ ХАДГАЛАХ':'Зээл хадгалах'}</button></div>`;
    setTimeout(renderLoanSchedulePreview,0);
  }

  function showLoanCreateFromDraft(){
    const draft=state.draft,input=scheduleInput();if(!draft)return;if(input){draft.method=input.method;draft.paymentDay=input.day;}openLoanForm(draft);
  }
  ...
  function paidEditRows(){return (editingLoan()?.installments||[]).filter(row=>row.status==='paid').sort((a,b)=>a.installment_number-b.installment_number);}
  function calculateSchedule(draft,method,paymentDay,paidRows=[]){
    const paidNumbers=new Set(paidRows.map(row=>Number(row.installment_number))),maxPaid=paidRows.reduce((max,row)=>Math.max(max,Number(row.installment_number)||0),0);
    if(draft.term<maxPaid)throw new Error(`Хугацааг ${maxPaid} сараас бага болгох боломжгүй. ${maxPaid}-р төлөлт төлөгдсөн байна.`);
    const numbers=[];for(let index=1;index<=draft.term;index++)if(!paidNumbers.has(index))numbers.push(index);
    const paidPrincipal=round2(paidRows.reduce((sum,row)=>sum+Number(row.principal_amount||0),0)),principal=round2(draft.principal-paidPrincipal),term=numbers.length;
    if(principal<0)throw new Error(`Авсан дүн нь төлсөн үндсэн дүн ${amount(paidPrincipal)}-ээс бага байж болохгүй.`);
    if(term===0){if(principal!==0)throw new Error('Төлөлтийн хугацааг нэмж үлдсэн дүнгийн хуваарь гаргана уу.');return [];}
    if(!(principal>0))throw new Error('Үлдсэн төлбөрийн дүн тэгээс их байх ёстой.');
    const monthlyRate=draft.interest/1200,annuity=monthlyRate===0?principal/term:principal*monthlyRate*Math.pow(1+monthlyRate,term)/(Math.pow(1+monthlyRate,term)-1),equalPrincipal=principal/term;
    let remaining=principal;const rows=[];
    numbers.forEach((installmentNumber,position)=>{
      ...
      if(position===numbers.length-1||principalPart>remaining)principalPart=round2(remaining);
      const total=round2(principalPart+interest);remaining=round2(Math.max(remaining-principalPart,0));
      rows.push({installment_number:installmentNumber,due_date:dueDate(draft.startDate,installmentNumber,paymentDay),principal_amount:principalPart,interest_amount:interest,total_amount:total,paid_amount:0,status:'pending'});
    });
    return rows;
  }

  function scheduleInput(){
    const method=fieldValue('loanRepaymentMethod')||state.draft?.method||'annuity',day=Number(fieldValue('loanPaymentDay'));
    if(!Number.isInteger(day)||day<1||day>31)return null;
    try{return {method,day,rows:calculateSchedule(state.draft,method,day,paidEditRows())};}catch(error){return {method,day,rows:[],error:error?.message||'Төлөлтийн хуваарь бодож чадсангүй.'};}
  }
  function renderLoanSchedulePreview(){
    ...
    if(input.error){target.innerHTML=`<div class="agreementWarning"><b>${esc(input.error)}</b></div>`;return;}
    const paid=paidEditRows(),paidTotal=paid.reduce((sum,row)=>sum+Number(row.total_amount||0),0),paidInterest=paid.reduce((sum,row)=>sum+Number(row.interest_amount||0),0),pendingTotal=input.rows.reduce((sum,row)=>sum+row.total_amount,0),pendingInterest=input.rows.reduce((sum,row)=>sum+row.interest_amount,0),total=paidTotal+pendingTotal,first=input.rows[0],isEdit=Boolean(editingLoan());
    const firstCard=first?`<div class="loanPreviewCard"><small>${isEdit?'Дараагийн':'Эхний'} төлөлт</small><b>${amount(first.total_amount)}</b><span>${dateLabel(first.due_date)}</span></div>`:'<div class="loanPreviewCard"><small>Төлөлт</small><b class="greenText">Дууссан</b><span>Үлдэгдэлгүй</span></div>';
    target.innerHTML=`${paid.length?`<div class="loanHistoryNotice"><b>${paid.length} төлөлтийн түүх хэвээр үлдэнэ</b><span>Зөвхөн үлдсэн ${input.rows.length} төлөлт дахин бодогдоно.</span></div>`:''}${firstCard}<div class="loanPreviewCard"><small>Нийт төлөх</small><b>${amount(total)}</b><span>Хүү ${amount(paidInterest+pendingInterest)}</span></div><div class="paymentSectionTitle"><span>${isEdit?'ДАРААГИЙН':'ЭХНИЙ'} 3 ТӨЛӨЛТ</span><small>${methodLabel(input.method)}</small></div><div class="card">${input.rows.slice(0,3).map(row=>`<div class="invoice"><div><small>${row.installment_number}-р төлөлт · ${dateLabel(row.due_date)}</small><b>${amount(row.total_amount)}</b></div><div class="sub">Үндсэн ${amount(row.principal_amount)}<br>Хүү ${amount(row.interest_amount)}</div></div>`).join('')||'<div class="center sub">Бүх төлөлт төлөгдсөн.</div>'}</div>`;
  }
  ...
  function loanScheduleChanged(loan,draft,input){
    return Math.abs(Number(loan.principal)-draft.principal)>.004
      ||Math.abs(Number(loan.annual_interest_rate)-draft.interest)>.00004
      ||String(loan.start_date)!==String(draft.startDate)
      ||Number(loan.term_months)!==draft.term
      ||Number(loan.payment_day)!==input.day
      ||String(loan.repayment_method)!==String(input.method);
  }

  async function saveLoan(){
    const input=scheduleInput(),draft=state.draft,storeId=activeStoreId(),client=sb(),button=document.getElementById('loanSaveBtn'),existing=editingLoan(),isEdit=Boolean(existing);
    if(!draft||!input)return notify('Төлөлтийн мэдээллээ шалгана уу.');if(input.error)return notify(input.error);if(!storeId||!client||!canManage())return notify('Дэлгүүрийн эрх олдсонгүй.');
    const rebuild=isEdit&&loanScheduleChanged(existing,draft,input),paidCount=isEdit?paidEditRows().length:0;
    if(rebuild&&paidCount&&!confirm(`${paidCount} төлөлтийн түүхийг хэвээр үлдээж, зөвхөн үлдсэн ${input.rows.length} төлөлтийг дахин бодох уу?`))return;
    if(button){button.disabled=true;button.textContent='Хадгалж байна...';}window.__nayadCriticalOperation=isEdit?'loan-edit':'loan-create';let loan=null,uploaded=[];
    try{
      ...
      if(isEdit){
        const {error}=await client.rpc('update_loan_with_schedule',{p_loan_id:existing.id,p_lender_type:draft.lenderType,p_lender_name:draft.lenderName,p_loan_name:draft.loanName,p_principal:draft.principal,p_annual_interest_rate:draft.interest,p_start_date:draft.startDate,p_term_months:draft.term,p_payment_day:input.day,p_repayment_method:input.method,p_rebuild_schedule:rebuild});
        if(error)throw error;closeLoanCreate();notify(rebuild?'Зээл засагдаж, үлдсэн төлөлтийн хуваарь шинэчлэгдлээ.':'Зээлийн мэдээлэл засагдлаа.');await syncLoans();window.render?.();return;
      }
      ...
      console.error(`NAYAD loan ${isEdit?'edit':'create'}:`,error);if(!isEdit&&uploaded.length)await client.storage.from(BUCKET).remove(uploaded).catch(()=>{});if(!isEdit&&loan?.id)await client.from('loans').delete().eq('id',loan.id).catch(()=>{});notify(error?.message||(isEdit?'Зээл засахад алдаа гарлаа.':'Зээл хадгалахад алдаа гарлаа.'));if(button){button.disabled=false;button.textContent=isEdit?'ӨӨРЧЛӨЛТ ХАДГАЛАХ':'Зээл хадгалах';}
    }finally{if(['loan-create','loan-edit'].includes(window.__nayadCriticalOperation))delete window.__nayadCriticalOperation;}
  }
  ...
  window.showLoanEdit=showLoanEdit;
  window.closeLoanCreate=closeLoanCreate;
  window.setLoanLenderType=setLenderType;
  window.renderLoanAnnualInterest=renderAnnualInterestPreview;
