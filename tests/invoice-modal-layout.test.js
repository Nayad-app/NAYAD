...
assert.match(paymentSource,/Падаан засах[\s\S]+classList\.add\('invoiceFormModal'\)/,'invoice edit must use the same centered modal backdrop');
assert.match(paymentSource,/Падаан засах[\s\S]+classList\.add\('invoiceFormSheet'\)/,'invoice edit must use the same compact modal sheet');
assert.match(paymentSource,/aria-label="Хаах"[^>]+cancelInvoiceEdit/,'invoice edit must expose the same safe close action');
