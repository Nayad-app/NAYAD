...
assert.match(sheetHtml,/Нэр 1/);
assert.match(sheetHtml,/Утасны дугаар 1/);
assert.match(sheetHtml,/Нэр 2/);
assert.match(sheetHtml,/Утасны дугаар 2/);
assert.doesNotMatch(sheetHtml,/<label for="newPhone">Утас \*<\/label>/,'organization form must remove the duplicate general phone field');
assert.match(sheetHtml,/<input id="newPhone" type="hidden"/,'legacy organization phone data must remain preservable');
assert.match(sheetHtml,/ногоон залгах товч энэ дугаар руу шууд залгана/);
assert.doesNotMatch(sheetHtml,/Регистр/);

context.showContactForm('organization',{id:99,name:'Эра жимс',contactType:'organization',phone:'70001122',logoPath:'store/logo.png',logoUrl:'https://example.test/logo.png'});
assert.match(sheetHtml,/Харилцагчийн мэдээлэл засах/);
assert.match(sheetHtml,/<input id="ePhone" type="hidden" value="70001122">/,'editing must preserve the hidden legacy organization phone');
...
data.companies.push({id:3,name:'Minimal',contactType:'organization',phone:'99112233',directorPhone:'88112233',salesPhone:'77112233',status:'active',invoices:[],debt:0});
...
assert.match(companiesHtml,/href="tel:88112233"/,'organization call button must use only phone number 1');
...
assert.doesNotMatch(missingPhoneHtml,/href="tel:70001234"/,'a missing phone number 1 must not fall back to the legacy organization phone');
assert.doesNotMatch(missingPhoneHtml,/href="tel:70005678"/,'a missing phone number 1 must not fall back to phone number 2');
...
assert.match(sheetHtml,/Утасны дугаар 1 бүртгэгдээгүй/);
assert.match(sheetHtml,/Утасны дугаар 1-ийг оруулна уу/);
...
assert.match(sheetHtml,/id="eDirectorPhone"/);
assert.match(sheetHtml,/Утасны дугаар 1/);
