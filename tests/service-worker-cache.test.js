...
assert.ok(cacheVersion>=128,'contact logos must invalidate the installed app shell');
...
assert.match(swSource,/\.\/contact-types\.js\?v=12/);
assert.match(indexSource,/\.\/contact-types\.js\?v=12/,'index and service worker must load the contact type module');
assert.match(swSource,/\.\/supplier-cloud\.js\?v=59/);
assert.match(indexSource,/\.\/supplier-cloud\.js\?v=59/,'index and service worker must load the same supplier code');
