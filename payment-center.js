...
(function(){
  ...
  let invoiceEditFiles=[];
  let invoiceDeleteCountdownTimer=null;
  let invoiceDeleteCountdownToken=0;
  ...
  function setInvoiceDeleteMode(active){
    document.getElementById('modal')?.classList?.toggle?.('invoiceDeleteModal',Boolean(active));
    document.getElementById('sheet')?.classList?.toggle?.('invoiceDeleteSheet',Boolean(active));
  }
  function resetInvoiceDeleteUi(){
    invoiceDeleteCountdownToken++;
    if(invoiceDeleteCountdownTimer!==null){clearInterval(invoiceDeleteCountdownTimer);invoiceDeleteCountdownTimer=null;}
    setInvoiceDeleteMode(false);
  }
  ...
  function companyV11(id){
    ...
    content.innerHTML=`<button class="back" onclick="page='companies';render()">← Буцах</button><div class="center"><div class="circle ${esc(company.color||'green')}" style="margin:auto;width:48px;height:48px">${esc((company.name||'N').slice(0,1).toUpperCase())}</div><h2 style="margin:8px 0 2px">${esc(company.name)}</h2><div class="bigAmount">${amount(company.debt)}</div><div class="sub">Баталгаажсан нийт өр</div></div>
      ...
      <div class="sectionTitle">Падаанууд</div><div class="card">${invoices.length?invoices.map(invoice=>{const status=invoice.status||'confirmed',balance=balanceOf(invoice);return `<div class="invoice invoiceClickable" role="button" tabindex="0" onclick="window.showInvoiceDetails('${esc(invoice.id)}')" onkeydown="if(event.key==='Enter'||event.key===' '){event.preventDefault();window.showInvoiceDetails('${esc(invoice.id)}')}"><div><small>${esc(invoice.date||'')} · ${esc(invoice.no||'Дугааргүй')}</small><b>${amount(invoice.amount)}</b><span class="invoiceStatus ${status}">${status==='cancelled'?'Цуцалсан':balance?'Төлөх өдөр '+dateLabel(dueOf(invoice)):'Төлөгдсөн'}</span></div><div><span class="${balance?'redText':'greenText'}">${balance?amount(balance):'Төлөгдсөн'}</span><small class="detailHint">Дэлгэрэнгүй →</small></div></div>`;}).join(''):'<div class="sub">Падаан алга.</div>'}</div>
  ...
  window.showInvoiceDetails=async function(invoiceId){
    ...
    open(`<h2>Падааны дэлгэрэнгүй</h2>
      <div class="invoiceDetailCard"><b>${esc(company.name)}</b><span>${esc(invoice.no||'Дугааргүй')}</span><div class="detailGrid"><small>Падааны огноо<b>${dateLabel(invoice.date)}</b></small><small>Төлөх хугацаа<b>${dateLabel(dueOf(invoice))}</b></small><small>Нийт дүн<b>${amount(invoice.amount)}</b></small><small>Үлдэгдэл<b class="${balance?'redText':'greenText'}">${amount(balance)}</b></small></div>${paid?`<div class="paidSummary"><span>Төлсөн дүн</span><b>${amount(paid)}</b></div>`:''}${discount?`<div class="agreementInfo">Хугацаандаа төлбөл ${discount}% хөнгөлөлт эдэлнэ${invoice.discount_deadline?' · '+dateLabel(invoice.discount_deadline)+' хүртэл':''}.</div>`:''}${invoice.note?`<div class="invoiceNote"><small>Нэмэлт тэмдэглэл</small><p>${esc(invoice.note)}</p></div>`:''}${Number(invoice.image_count)||invoice.image_url?`<button class="secondary full invoiceImageButton" onclick="window.viewInvoiceImages('${esc(invoice.id)}')">▧ Падааны зураг үзэх</button>`:''}</div>
      ${status==='confirmed'?`<p class="sub">Падааны мэдээллийг хүссэн үедээ засна. Өмнө бүртгэсэн төлбөр өөрчлөгдөхгүй.</p><div class="invoiceDetailActions"><button class="primary" onclick="window.editConfirmedInvoice('${esc(invoice.id)}')">✎ Падаан засах</button><button class="invoiceDeleteButton" onclick="window.showInvoiceDeleteConfirm('${esc(invoice.id)}')"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M4 7h16M9 7V4h6v3M7 7l1 13h8l1-13M10 11v5M14 11v5"/></svg><span>Устгах</span></button></div>`:`<div class="agreementWarning"><b>Засах боломжгүй</b><span>Цуцалсан падааныг засах боломжгүй.</span></div><button class="invoiceDeleteButton full invoiceDeleteStandalone" onclick="window.showInvoiceDeleteConfirm('${esc(invoice.id)}')"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M4 7h16M9 7V4h6v3M7 7l1 13h8l1-13M10 11v5M14 11v5"/></svg><span>Падаан устгах</span></button>`}
      ${paid?paymentReversalRows(postedPayments,invoiceId):''}
      <button class="secondary full" style="margin-top:9px" onclick="closeSheet()">Хаах</button>`);
  };

  window.showInvoiceDeleteConfirm=function(invoiceId){
    const invoice=invoiceById(invoiceId),company=companyForInvoice(invoiceId);if(!invoice||!company)return;
    resetInvoiceDeleteUi();
    open(`<div class="invoiceDeleteIcon"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M4 7h16M9 7V4h6v3M7 7l1 13h8l1-13M10 11v5M14 11v5"/></svg></div><h2>Падаан устгах уу?</h2><div class="invoiceDeleteSummary"><span>${esc(invoice.no||'Дугааргүй')}</span><b>${amount(invoice.amount)}</b></div><p class="invoiceDeleteWarning">Энэ падаан болон түүнтэй холбоотой зураг, төлбөр, засварын түүх бүрмөсөн устна.</p><b class="invoiceDeleteIrreversible">Буцаах боломжгүй.</b><div class="actions"><button class="secondary" onclick="window.cancelInvoiceDelete('${esc(invoice.id)}')">Болих</button><button id="confirmInvoiceDeleteBtn" class="invoiceDeleteConfirmButton" disabled onclick="window.deleteInvoiceWithHistory('${esc(invoice.id)}')">УСТГАХ (2)</button></div>`);
    setInvoiceDeleteMode(true);
    const token=++invoiceDeleteCountdownToken;
    let remaining=2;
    invoiceDeleteCountdownTimer=setInterval(()=>{
      if(token!==invoiceDeleteCountdownToken){clearInterval(invoiceDeleteCountdownTimer);invoiceDeleteCountdownTimer=null;return;}
      remaining--;
      const button=document.getElementById('confirmInvoiceDeleteBtn');
      if(remaining>0){if(button)button.textContent=`УСТГАХ (${remaining})`;return;}
      clearInterval(invoiceDeleteCountdownTimer);invoiceDeleteCountdownTimer=null;
      if(button){button.disabled=false;button.textContent='УСТГАХ';}
    },1000);
  };

  window.cancelInvoiceDelete=function(invoiceId){resetInvoiceDeleteUi();window.showInvoiceDetails(invoiceId);};

  function removeInvoiceFromLocalState(invoiceId){
    try{
      const next=typeof data!=='undefined'?data:state();
      for(const company of next.companies||[]){
        company.invoices=(company.invoices||[]).filter(invoice=>String(invoice.id)!==String(invoiceId));
      }
      financeSync();
    }catch(error){console.warn('Invoice local cleanup:',error);}
  }

  window.deleteInvoiceWithHistory=async function(invoiceId){
    const invoice=invoiceById(invoiceId),company=companyForInvoice(invoiceId);if(!invoice||!company)return;
    if(window.__nayadCriticalOperation){notify('Өмнөх үйлдэл дууссаны дараа дахин оролдоно уу.');return;}
    const button=document.getElementById('confirmInvoiceDeleteBtn');if(button){button.disabled=true;button.textContent='Устгаж байна...';}
    const operationToken='invoice-delete:'+crypto.randomUUID(),storeId=activeStoreId(),companyId=company.id;
    window.__nayadCriticalOperation=operationToken;
    let remoteComplete=false,storageCleanupFailed=false;
    try{
      const {data:result,error}=await sb().rpc('delete_invoice_with_history',{p_invoice_id:invoiceId});
      if(error)throw error;
      remoteComplete=true;
      const row=Array.isArray(result)?result[0]:result;
      const paths=[...(Array.isArray(row?.storage_paths)?row.storage_paths:[]),...(Array.isArray(invoice.image_paths)?invoice.image_paths:[]),invoice.image_path].filter(Boolean);
      try{await removeInvoiceStoragePaths(paths);}catch(error){storageCleanupFailed=true;console.warn('Deleted invoice image cleanup:',error);}
      removeInvoiceFromLocalState(invoiceId);
      resetInvoiceDeleteUi();close();
      try{await window.__nayadStartCloudSync?.({reason:'invoice-deleted',force:true});}catch(error){console.warn('Invoice delete refresh:',error);}
      if(String(activeStoreId()||'')===String(storeId||'')&&companyById(companyId))window.company(companyId);
      else try{render();}catch(_){ }
      notify(storageCleanupFailed?'Падаан устлаа. Зарим зургийг цэвэрлэхэд алдаа гарлаа.':'Падаан болон холбогдох түүх устгагдлаа.');
    }catch(error){
      console.error('Invoice delete:',error);
      notify(error?.message||'Падаан устгахад алдаа гарлаа.');
      if(button&&!remoteComplete){button.disabled=false;button.textContent='УСТГАХ';}
    }finally{if(window.__nayadCriticalOperation===operationToken)delete window.__nayadCriticalOperation;}
  };
  ...
  function clearInvoiceEditFiles(){
    for(const item of invoiceEditFiles){if(item.preview)URL.revokeObjectURL(item.preview);}
    invoiceEditFiles=[];
  }

  function renderInvoiceEditPreviews(){
    const target=document.getElementById('invoiceEditPreviews');if(!target)return;
    target.innerHTML=invoiceEditFiles.map((item,index)=>`<div class="invoiceEditPreview"><img src="${esc(item.preview)}" alt="Сонгосон зураг ${index+1}"><button type="button" onclick="window.removeInvoiceEditFile(${index})" aria-label="Зураг хасах">×</button><small>${index+1}</small></div>`).join('');
  }

  window.selectInvoiceEditFiles=function(files){
    clearInvoiceEditFiles();
    const images=Array.from(files||[]).filter(file=>String(file.type||'').startsWith('image/')).slice(0,20);
    invoiceEditFiles=images.map(file=>({file,preview:URL.createObjectURL(file)}));
    renderInvoiceEditPreviews();
    if((files?.length||0)>20)notify('Нэг падаанд хамгийн ихдээ 20 зураг оруулна.');
  };

  window.removeInvoiceEditFile=function(index){
    const [removed]=invoiceEditFiles.splice(index,1);if(removed?.preview)URL.revokeObjectURL(removed.preview);renderInvoiceEditPreviews();
  };

  window.cancelInvoiceEdit=function(invoiceId){clearInvoiceEditFiles();window.showInvoiceDetails(invoiceId);};

  async function removeInvoiceStoragePaths(paths){
    const clean=Array.from(new Set((paths||[]).filter(Boolean)));if(!clean.length)return;
    const {error}=await sb().storage.from('invoice-images').remove(clean);if(error)throw error;
  }
  ...
  window.saveConfirmedInvoiceRevision=async function(invoiceId){
    ...
    const discountDeadline=input('reviseInvoiceDiscountDeadline')||null,note=input('reviseInvoiceNote').trim()||null,paid=Number(invoice.paid)||0;
    ...
    if(invoiceAmount<paid){notify(`Нийт дүнг төлсөн ${amount(paid)}-өөс бага болгох боломжгүй.`);return;}
    if(dueDate<invoiceDate){notify('Төлөх хугацаа падааны өдрөөс өмнө байж болохгүй.');return;}
    if(discountPercent<0||discountPercent>=100){notify('Хөнгөлөлтийн хувь 0–99.99 хооронд байна.');return;}
    ...
    const operationToken='invoice-edit:'+crypto.randomUUID();
    window.__nayadCriticalOperation=operationToken;
    const uploadedPaths=[],newImages=[];let remoteComplete=false;
    try{
      ...
      for(let index=0;index<invoiceEditFiles.length;index++){
        ...
        newImages.push({image_url:imageUrl,image_path:path,page_number:index+1});
      }
      const {error}=await sb().rpc('edit_confirmed_invoice',{p_invoice_id:invoiceId,p_invoice_no:invoiceNo||null,p_invoice_date:invoiceDate,p_due_date:dueDate,p_amount:invoiceAmount,p_discount_percent:discountPercent,p_discount_deadline:discountDeadline,p_note:note,p_images:newImages.length?newImages:null});
      if(error)throw error;
      remoteComplete=true;
      const oldPaths=Array.isArray(invoice.image_paths)?invoice.image_paths:(invoice.image_path?[invoice.image_path]:[]);
      if(newImages.length&&oldPaths.length){try{await removeInvoiceStoragePaths(oldPaths);}catch(error){console.warn('Old invoice images cleanup:',error);}}
      clearInvoiceEditFiles();
      close();
      try{await window.__nayadStartCloudSync?.({reason:'confirmed-invoice-edited',force:true});}catch(error){console.warn('Invoice edit refresh:',error);}
      try{render();}catch(_){}
      notify('Падааны өөрчлөлт хадгалагдлаа.');
    }catch(error){
      console.error('Invoice edit:',error);
      if(!remoteComplete&&uploadedPaths.length){try{await removeInvoiceStoragePaths(uploadedPaths);}catch(cleanupError){console.warn('Invoice edit compensation:',cleanupError);}}
      notify(remoteComplete?'Падаан хадгалагдсан. Мэдээллээ дахин ачаална уу.':(error?.message||'Падаан засахад алдаа гарлаа.'));if(button&&!remoteComplete){button.disabled=false;button.textContent='ӨӨРЧЛӨЛТИЙГ ХАДГАЛАХ';}
    }
    finally{if(window.__nayadCriticalOperation===operationToken)delete window.__nayadCriticalOperation;}
  };
  ...
  nightStyle.textContent=`
    ...
    html.nightMode .emptyPay,html.nightMode .paySupplierCard,html.nightMode .invoiceDetailCard{background:#1D1D1B;border-color:#3C3C38}html.nightMode .invoiceStatus{background:#30302D;color:#D8D8D0}html.nightMode .invoiceStatus.cancelled{background:#472426;color:#FFD0D0}
    html.nightMode .detailGrid small b,html.nightMode .confirmPay b,html.nightMode .invoicePaidGrid b{color:#F4F4EF}html.nightMode .paidSummary{border-color:#3C3C38}html.nightMode .invoiceNote,html.nightMode .invoicePaidGrid{background:#252523}html.nightMode .invoiceDeleteButton{background:#252523;border-color:#8C4848;color:#FF9A9A}html.nightMode .invoiceDeleteSummary{background:#252523}html.nightMode .invoiceDeleteSummary b{color:#F4F4EF}html.nightMode .invoiceDeleteIcon{background:#472426;color:#FFD0D0}html.nightMode .invoiceEditImageRow{border-color:#4A4A45;background:#252523}html.nightMode .allocationAmount{background:#252523;color:#F4F4EF;border-color:#4A4A45}html.nightMode .agreementWarning{background:#3A2022;border-color:#743F43}html.nightMode .agreementWarning span{color:#FFD0D0}html.nightMode .agreementInfo{background:#31280D;border-color:#715D18;color:#FFE39A}html.nightMode .notificationRow{background:#1D1D1B;border-color:#3C3C38}html.nightMode .notificationRow.overdue{background:#3A2022;border-color:#743F43}
  `;
  ...
  document.addEventListener('click',event=>{if(event.target?.id==='modal')resetInvoiceDeleteUi();});
  document.addEventListener('keydown',event=>{if(event.key==='Escape')resetInvoiceDeleteUi();});
})();
