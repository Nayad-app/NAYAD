...
(function(){
  ...
  function invoiceRow(item,index){
    ...
    return `<article class="dueRow ${classes}">
  ...
  function paymentCenter(){
    ...
    return `<div class="paymentHead"><div><div class="hello">Өглөгийн удирдлага</div><div class="name">Төлбөрийн төв</div></div><button class="noticeButton" onclick="showNotificationCenter()" aria-label="Мэдэгдэл">🔔${unreadNotifications.length?`<i>${unreadNotifications.length}</i>`:''}</button></div>
  ...
  function companyV11(id){
    ...
    content.innerHTML=`<button class="back" onclick="page='companies';render()">← Буцах</button><div class="center"><div class="circle ${esc(company.color||'green')}" style="margin:auto;width:48px;height:48px">${esc((company.name||'N').slice(0,1).toUpperCase())}</div><h2 style="margin:8px 0 2px">${esc(company.name)}</h2><div class="bigAmount">${amount(company.debt)}</div><div class="sub">Баталгаажсан нийт өр</div></div>
      ...
      <div class="sectionTitle">Падаанууд</div><div class="card">${invoices.length?invoices.map(invoice=>{const status=invoice.status||'confirmed',balance=balanceOf(invoice);return `<div class="invoice"><div><small>${esc(invoice.date||'')} · ${esc(invoice.no||'Дугааргүй')}</small><b>${amount(invoice.amount)}</b><span class="invoiceStatus ${status}">${status==='draft'?'Ноорог':status==='cancelled'?'Цуцалсан':balance?'Төлөх өдөр '+dateLabel(dueOf(invoice)):'Төлөгдсөн'}</span></div><div>${status==='draft'?`<button class="secondary" onclick="window.invoice('${esc(company.id)}','${esc(invoice.id)}')">Засах</button>`:`<span class="${balance?'redText':'greenText'}">${balance?amount(balance):'Төлөгдсөн'}</span>`}</div></div>`;}).join(''):'<div class="sub">Падаан алга.</div>'}</div>
